...
function start(httpServer, config) {
  ...
  _wss.on('connection', (ws, req) => {
    ...
    const apiKey = aCfg?.llm?.openrouterKey || process.env.OPENROUTER_API_KEY || null;
    ...
    if (!aCfg || !apiKey) {
      ws.send(JSON.stringify({
        ...
        status:         sync.status,
        reason:         !aPath ? 'agentDataPath not configured' : !aCfg ? 'agent config not found' : 'agent has no LLM key (set llm.openrouterKey in agent config)',
      }));
      return;
    }
