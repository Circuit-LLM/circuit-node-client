...
function getStatus(dataPath) {
  ...
  const sess      = readJson(dataPath, 'session_strategy.json') ?? {};
  const pausedRaw = readJson(dataPath, 'trading_paused.json');
  const strat     = (readConfig(dataPath) ?? {}).strategy ?? {};
  ...
  // Realized performance across all recorded (closed) trades — those with a pnlSol.
  const closed = Array.isArray(history) ? history.filter(t => t.pnlSol != null) : [];
  const wins   = closed.filter(t => t.pnlSol > 0).length;
  const realizedPnlSol = closed.reduce((s, t) => s + t.pnlSol, 0);
  ...
  return {
    ...
    // Realized performance across the agent's trade history.
    stats: {
    ...
    // Pause state — the agent writes trading_paused.json when it halts buys (e.g. low SOL).
    paused: pausedRaw?.paused === true ? { reason: pausedRaw.reason ?? null, since: pausedRaw.pausedAt ?? null } : null,
    // The session strategy the agent is currently operating under (set by its reflection loop).
    strategy: {
      mode:          sess.mode ?? null,
      goal:          sess.sessionGoal ?? null,
      reasoning:     sess.reasoning ?? null,
      patternFilter: sess.patternFilter ?? [],
      minScore:      sess.minScoreOverride ?? strat.minScanScore ?? null,
      buysUsed:      sess.buysThisSession ?? null,
      buysMax:       sess.maxBuysThisSession ?? null,
      expiresAt:     sess.expiresAt ?? null,
    },
    // The risk config the agent trades with.
    config: {
      entryBudgetSol:   strat.entryBudgetSol ?? null,
      stopLossPct:      strat.stopLossPct ?? null,
      takeProfitPct:    strat.takeProfitPct ?? null,
      maxHoldMinutes:   strat.maxHoldMinutes ?? null,
      maxOpenPositions: strat.maxOpenPositions ?? null,
      minLiquidity:     strat.minLiquidity ?? null,
      trailingStop:     strat.trailingStopActivatePct != null
        ? `+${strat.trailingStopActivatePct}% act / ${strat.trailingStopDistancePct}% trail` : null,
    },
