...
function _spawn() {
  ...
  // ...and ENFORCE that. advertising sandbox=node only turns away 'oci' bundles: the scheduler gates a
  // bundle on sandbox rank alone, so a third-party publisher who declares runtime:'node' in their own
  // manifest satisfies this host and would run as an UNSANDBOXED SAME-UID process — able to read
  // data/identity.json (mode 600, our uid) and sign as this node, which buys free gateway inference and
  // our registry identity. This box lends capacity to other owners' agents, so it runs built-in
  // workloads only and never a node-runtime bundle. Third-party code must be oci-sandboxed.
  if (!env.CIRCUIT_ALLOW_NODE_BUNDLES) env.CIRCUIT_ALLOW_NODE_BUNDLES = '0';
