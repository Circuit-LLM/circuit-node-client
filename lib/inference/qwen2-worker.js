...
class Qwen2Worker {
  ...
  _ensureLoaded() {
    ...
    for (let l = layerStart; l <= layerEnd; l++) {
      ...
      // Bias tensors are optional (present in Qwen2, absent in Llama/Mistral)
      const getOpt = (suffix, n) => {
        const t = this._rawMap.get(`blk.${l}.${suffix}`);
        if (!t) return null;
        return dequantize(t.data, t.ggmlType, n);
      };
      const { headDim: hd } = this._dims();
      layers[l] = {
        attn_norm:    get('attn_norm.weight'),
        ffn_norm:     get('ffn_norm.weight'),
        attn_q_w:     get('attn_q.weight'),
        attn_q_b:     getOpt('attn_q.bias',  numHeads   * hd),
        attn_k_w:     get('attn_k.weight'),
        attn_k_b:     getOpt('attn_k.bias',  numKvHeads * hd),
        attn_v_w:     get('attn_v.weight'),
        attn_v_b:     getOpt('attn_v.bias',  numKvHeads * hd),
  ...
  forward(hidden, pos) {
    ...
    const maxSeq = this.cfg.maxSeqLen ?? 4096;

    if (!hidden || hidden.length !== hiddenDim) {
      throw new Error(`forward: hidden shape mismatch — expected ${hiddenDim}, got ${hidden?.length}`);
    }
    if (pos < 0 || pos >= maxSeq) {
      throw new Error(`forward: pos ${pos} out of range [0, ${maxSeq})`);
    }
    ...
    for (let l = layerStart; l <= layerEnd; l++) {
      ...
      // Add biases (null when model has no attention biases, e.g. Llama)
      if (w.attn_q_b) for (let i = 0; i < numHeads   * headDim; i++) q[i] += w.attn_q_b[i];
      if (w.attn_k_b) for (let i = 0; i < numKvHeads * headDim; i++) k[i] += w.attn_k_b[i];
      if (w.attn_v_b) for (let i = 0; i < numKvHeads * headDim; i++) v[i] += w.attn_v_b[i];
      ...
      if (kvSlot + numKvHeads * headDim > this._kvK[l].length) {
        throw new Error(`KV write out of bounds at pos ${pos}, layer ${l}`);
      }
      ...
      for (let h_idx = 0; h_idx < numHeads; h_idx++) {
        // Proportional mapping: always in [0, numKvHeads) even when numHeads % numKvHeads != 0
        const kvHead = Math.floor(h_idx * numKvHeads / numHeads);
