...
// ── Status ────────────────────────────────────────────────────────────────────
function status() {
  // Surface the configured contribution size so dashboards can show "layers held" (the python
  // worker_state.json may not echo it back). Only when we actually own a worker config.
  const layers = _config?.capacityLayers;
  return { ..._state, ...(layers ? { capacityLayers: layers } : {}) };
}
