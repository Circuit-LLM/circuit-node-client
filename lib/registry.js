...
async function announce(config, extraFields = {}) {
  ...
  const body = {
    ...
    shards:       (config.node.shards && config.node.shards.length) ? config.node.shards : ['all'],
    region:       config.node.region ?? 'unknown',
    agentRunning: config.node.agentEnabled ?? false,
    apiPort:      config.node.apiPort ?? null,
    // wallet (Solana payout) is OMITTED when unset — a `wallet: undefined` field gets signed but
    // dropped by JSON.stringify on the wire, so the server rebuilds a different payload and the
    // announce fails with "Invalid node signature". Only include it when there's a real value.
    ...(config.node.payoutWallet ? { wallet: config.node.payoutWallet } : {}),
    ...extraFields,
  };
...
function startHeartbeat(config, getStatus = async () => ({})) {
  ...
  const baseMs       = config.heartbeat?.intervalMs   ?? 60_000;
  const maxBackoffMs = config.heartbeat?.maxBackoffMs ?? 900_000; // cap retries at 15 min

  // Self-scheduling loop (not setInterval) so the delay can grow while the registry is down.
  const tick = async () => {
    if (_hbStopped) return;
    let ok = false;
    try {
      const status = await getStatus();
      ok = !!(await ping(config, status));
    } catch (err) {
      console.error('[registry] Heartbeat error:', err.message);
    }
    if (_hbStopped) return;

    // Steady cadence when healthy; exponential backoff with jitter while unreachable, so a
    // fleet of nodes doesn't synchronise and thundering-herd the registry on recovery.
    _consecutiveFailures = ok ? 0 : _consecutiveFailures + 1;
    let delay = baseMs;
    if (_consecutiveFailures > 0) {
      const backoff = Math.min(baseMs * 2 ** (_consecutiveFailures - 1), maxBackoffMs);
      delay = Math.min(backoff * (0.5 + Math.random()), maxBackoffMs); // 0.5×–1.5× jitter
    }
    _timer = setTimeout(tick, delay);
    _timer.unref();
  };

  _timer = setTimeout(tick, baseMs);
  _timer.unref();
  console.log(`[registry] Heartbeat started (interval: ${baseMs}ms, backoff cap: ${maxBackoffMs}ms)`);
}
