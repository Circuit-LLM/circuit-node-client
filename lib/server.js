...
function start(config, getPeers = async () => []) {
  ...
  // localhost coordinator → the configured coordinator (if any) → the public gateway ALWAYS last.
  // The public gateway is where a live node gets free inference AND the OpenRouter fallback when the
  // mesh is down, so it must stay in the list even when coordinatorUrl is set — otherwise a stale or
  // dead coordinatorUrl (e.g. an old RunPod direct-TCP port that remaps on restart) strands the chat
  // with no route to the fallback. Deduped so a coordinatorUrl equal to the public gateway isn't tried twice.
  const _inferUrls = () => {
  ...
  // Pair (or unpair) a local circuit-agent by pointing the node at its data dir — enables the
  // dashboard's Agent + Chat tabs. Localhost only. Pass dataPath: null/"" to unpair. A restart is
  // recommended so the chat WebSocket picks it up; the desktop app triggers one automatically.
  app.post('/setup/agent', (req, res) => {
    if (!_adminLocal(req, res)) return;
    const raw = req.body?.dataPath;
    if (raw === null || raw === '' || raw === undefined) {
      _saveConfig({ node: { agentDataPath: null } });
      return res.json({ ok: true, paired: false });
    }
    if (typeof raw !== 'string') return res.status(400).json({ ok: false, error: 'dataPath must be a string' });
    const abs = path.resolve(raw);
    let st; try { st = fs.statSync(abs); } catch { return res.status(400).json({ ok: false, error: 'that folder does not exist' }); }
    if (!st.isDirectory()) return res.status(400).json({ ok: false, error: 'that path is not a folder' });
    const agentFound = fs.existsSync(path.join(abs, 'agent-identity.json'));
    _saveConfig({ node: { agentDataPath: abs } });
    res.json({ ok: true, paired: true, agentDataPath: abs, agentFound, restartRecommended: true });
  });
  ...
  app.post('/inference/chat', async (req, res) => {
    const { messages = [], max_tokens = 1024, temperature = 0.7, stream = true } = req.body ?? {};
    // Streaming SSE is unreliable to consume inside the embedded desktop webview (the body never
    // arrives, leaving an empty reply), so the dashboard asks for stream:false and we return one
    // buffered JSON completion. Default stays true for streaming CLI/browser callers.
    ...
    const bodyObj = { messages: msgs, max_tokens, temperature, stream: wantStream };
    ...
    for (const url of urls) {
      ...
      // Non-streaming: buffer the whole completion and return it as one JSON body. Forward the
      // backend tag (mesh vs openrouter-fallback) so the client can show what answered.
      if (!wantStream) {
        const bodyText = await upstream.text();
        const backend = upstream.headers.get('x-circuit-backend');
        if (backend) res.setHeader('X-Circuit-Backend', backend);
