...
function start(config, getPeers = async () => []) {
  ...
  // Tries localhost:19200 (free for co-located coordinator) first, then
  // falls back to config.llmWorker.coordinatorUrl.
  ...
  app.post('/setup/install/cpu', (req, res) => {
    ...
    try {
      const src = path.join(require('os').homedir(), 'circuit-agent-cloud', 'node-host');
      const destBase = path.join(__dirname, '..', 'vendor');
      if (fs.existsSync(path.join(src, 'host.js'))) {
        fs.cpSync(src, path.join(destBase, 'node-host'), { recursive: true });
      ...
      res.status(409).json({ ok: false, error: 'node-host source not found',
        hint: 'Bundle circuit-agent-cloud/node-host into the node-client (vendor/node-host) to enable one-click CPU hosting.' });
  ...
  app.post('/cloud/host/start', (req, res) => {
    ...
    try {
      ...
      _saveConfig({ agentCloud: { enabled: true, maxAgents: budget.maxAgents, maxCpu: budget.maxCpu,
      ...
      if (upstream.status === 402) {
        if (url === urls[urls.length - 1]) {
          return res.status(402).json({
            ...
            message: 'The inference coordinator is not available on localhost. Run the coordinator co-located with this node for free inference.',
          });
        }
        continue;
      }
