...
function _stakePoolUrl(config) {
  ...
  const ok = id && !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(id); // not a base58 address → treat as a web id
...
function start(config, getPeers = async () => []) {
  ...
  app.get('/gpu/status', (req, res) => { try { res.json(gpuHost.status()); } catch (e) { res.status(500).json({ error: e.message }); } });
  ...
  app.post('/inference/chat', async (req, res) => {
    ...
    const SYS = 'You are Circuit, the assistant for Circuit LLM — a decentralized AI network served across a mesh of GPUs. Always respond in English unless the user explicitly writes to you in another language. Be concise, accurate, and helpful.';
