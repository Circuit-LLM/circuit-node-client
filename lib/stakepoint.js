...
async function verifyStake(walletAddress, poolAddress, minAmount, decimals, rpcUrl) {
  try {
    ...
    // Compare against minimum using BigInt arithmetic — no floating-point errors
    const minAmountRaw = BigInt(Math.round((minAmount ?? 0) * div));
...
module.exports = { getStakePositions, verifyStake, verifyStakeCached, cacheStats, PROGRAM_ID, DISCRIMINATOR };
