...
async function _applyViaTarball(config, newVersion, tarballUrl) {
  ...
  console.log('[updater] Downloading tarball from GitHub…');
  try {
    const token = config.updates?.githubToken ?? null;
    const headers = { 'User-Agent': `circuit-node-client/${CURRENT_VERSION}` };
    if (token) headers['Authorization'] = `token ${token}`;
    const res = await fetch(tarballUrl, { headers, signal: AbortSignal.timeout(120_000) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const buf = Buffer.from(await res.arrayBuffer());
    console.log(`[updater] Downloaded ${(buf.length / 1024).toFixed(1)} KB`);
    fs.mkdirSync(STAGE_DIR, { recursive: true });
    fs.writeFileSync(ARCHIVE, buf);
  } catch (err) {
    console.error('[updater] Tarball download failed:', err.message);
    _cleanup(STAGE_DIR);
    return false;
  }
