...
//   2. PRIMARY: download + verify the SIGNED update package (Ed25519 manifest + sha256).
//      Fails CLOSED if no signing key / bad signature / checksum mismatch — never runs
//      unverified code.
//   3. Fallback: plain `git pull --ff-only` runs UNVERIFIED code, so it is gated behind
//      config.updates.allowUnsignedGit (default OFF). A repo or push-token compromise must
//      not be able to ship unsigned code to field nodes via git.
...
const APP_ROOT        = path.join(__dirname, '..');   // CODE root — rsync/tar/git target (may be read-only in the app)
const UPDATE_HISTORY  = path.join(home.DATA_DIR, 'update-history.json');   // writable state → CIRCUIT_NODE_HOME aware
...
async function checkForUpdate(config) {
  ...
  // Releases are tagged `desktop-vX.Y.Z` (see .github/workflows/desktop-build.yml); older/manual
  // tags may be `vX.Y.Z` or bare `X.Y.Z`. Strip an optional `desktop-` and `v` before comparing.
  // A tag that isn't a version parses to 0.0.0 in the local semver helper, so gt() just reads it as
  // "no update" rather than throwing — no extra validity guard needed.
...
async function applyUpdate(config, newVersion, tarballUrl) {
  ...
  const BACKUP_DIR = path.join(home.DATA_DIR, 'backups', CURRENT_VERSION);
  try {
  ...
  // Primary: verified, signed tarball (supply-chain gate). _applyViaTarball fails CLOSED
  // without a signing key / on bad signature / on checksum mismatch — so auto-apply that
  // can't be verified simply does not happen (safe default: stay on current code).
  const ok = await _applyViaTarball(config, newVersion, tarballUrl);
  if (ok) {
    _logHistory({ version: newVersion, status: 'applied', method: 'tarball' });
    console.log(`[updater] v${newVersion} applied via verified package — restarting`);
    setTimeout(() => process.exit(0), 500);
    return;
  }

  // Fallback: plain `git pull` runs UNVERIFIED code. Only when the operator explicitly opts
  // in (trusted/dev clones) — NEVER by default — so a repo/push-token compromise can't push
  // unsigned code to field nodes through the update channel.
  if (config.updates?.allowUnsignedGit && _applyViaGit()) {
    _logHistory({ version: newVersion, status: 'applied', method: 'git-pull(unverified)' });
    console.log(`[updater] v${newVersion} applied via git pull (UNVERIFIED, operator opt-in) — restarting`);
    setTimeout(() => process.exit(0), 500);
    return;
  }

  console.error('[updater] No verified update applied (signed tarball required; set updates.allowUnsignedGit=true to permit unverified git pull)');
  _logHistory({ version: newVersion, status: 'failed', reason: 'verified tarball unavailable/failed; git-pull disabled or failed' });
}
...
async function _fetchSignedManifest(config) {
  // The registry URL lives at config.network.registryUrl everywhere else in the client
  // (server.js/sync.js/registry.js/node-client.js all use it). Read it there; keep a
  // top-level config.registryUrl as an explicit override for backward compat.
  const base = String(config.network?.registryUrl || config.registryUrl || '').replace(/\/$/, '');
  if (!base) throw new Error('no registryUrl configured (set network.registryUrl)');
...
async function _applyViaTarball(config, newVersion, tarballUrl) {
  const STAGE_DIR = path.join(home.DATA_DIR, 'staging', newVersion);
...
function rollback(targetVersion) {
  const archive = path.join(home.DATA_DIR, 'backups', targetVersion, 'backup.tar.gz');
...
function getBackups() {
  const dir = path.join(home.DATA_DIR, 'backups');
